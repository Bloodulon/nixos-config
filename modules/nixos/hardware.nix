...
{ pkgs, ... }: {
  hardware = { # Параметры для 24.05 и unstable могут сильно отличаться
    ...
    hardware.graphics = { # hardware.opengl переименован в hardware.graphics в unstable ветке
      enable = true;
    ...
    hardware.opentabletdriver.enable = true; # Установить, настроить и добавить в автозапуск otd

    hardware.keyboard.qmk.enable = true; # Еnable non-root access to the firmware of QMK keyboards.
